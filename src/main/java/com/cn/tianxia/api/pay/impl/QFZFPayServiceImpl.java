...
/**
 *
 * @ClassName QFZFPayServiceImpl
 * @Description TODO(趣付支付)
 * @author Bing
 * @Date 2019年5月4日 下午12:12:28
 * @version 1.0.0
 */

public class QFZFPayServiceImpl extends PayAbstractBaseService implements PayService {

    /**
     * 日志
     */
    private static final Logger logger = LoggerFactory.getLogger(QFZFPayServiceImpl.class);
     ...
     * 构造器，初始化参数
     */
    public QFZFPayServiceImpl() {
    }

    public QFZFPayServiceImpl(Map<String, String> data) {
    ...
    public JSONObject smPay(PayEntity entity) {
        try {
            ...
            logger.info("[QFZF]趣付扫码支付请求参数报文:{}", data);

            String response = toPostForm(data, payUrl);
            if (StringUtils.isBlank(response)) {
                logger.error("[QFZF]趣付扫码支付下单失败：HTTP请求无响应");
                PayResponse.error("[QFZF]趣付扫码支付下单失败：HTTP请求无响应");
            }
            JSONObject jsonObject = JSONObject.fromObject(response);
            logger.info("[QFZF]趣付扫码支付发起HTTP请求响应结果:{}", jsonObject);
            ...
            e.printStackTrace();
            return PayResponse.error("[QFZF]趣付扫码支付异常" + e.getMessage());
        }
    }

    private boolean verifyCallback(Map<String, String> data) {
        logger.info("[QFZF]趣付扫码支付回调验签开始==============START===========");

        //获取回调通知原签名串
        String sourceSign = data.remove("sign");
        logger.info("[QFZF]趣付扫码支付回调验签获取原签名串:{}", sourceSign);
        //生成验签签名串
        String sign = null;
        try {
            sign = generatorSign(data);
            logger.info("[QFZF]趣付扫码支付回调验签生成加密串:{}", sign);
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[QFZF]趣付扫码支付回调验签生成加密串异常:{}", e.getMessage());
        }
    ...
    private Map<String,String> sealRequest(PayEntity entity) throws Exception {
        logger.info("[QFZF]趣付扫码支付封装请求参数开始=====================START==================");
        try {
            //创建支付请求参数存储对象
...
//            sign	Y	string	Y	请参考签名章节
            ...
            e.printStackTrace();
            logger.error("[QFZF]趣付扫码支付封装请求参数异常:" + e.getMessage());
            throw new Exception("[QFZF]趣付扫码支付封装支付请求参数异常!");
        }
    }
    ...
    public String generatorSign(Map<String, String> data) throws Exception {
        logger.info("[QFZF]趣付扫码支付生成支付签名串开始==================START========================");
        try {
            ...
            logger.info("[QFZF]趣付扫码支付生成待签名串:" + signStr);
            //生成加密串
            String sign = MD5Utils.md5toUpCase_32Bit(signStr);
            logger.info("[QFZF]趣付扫码支付生成加密签名串:" + sign);
            return sign;
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("[QFZF]趣付扫码支付生成支付签名串异常:" + e.getMessage());
            throw new Exception("[QFZF]趣付扫码支付生成支付签名串异常!");
        }
    }
    ...
    public String notify(HttpServletRequest request, HttpServletResponse response, JSONObject config) {
        ...
        logger.info("[QFZF]趣付扫码支付回调请求参数:{}", JSONObject.fromObject(infoMap));
        if (!MapUtils.isNotEmpty(infoMap)) {
            logger.error("[QFZF]趣付扫码支付获取回调请求参数为空");
            return ret__failed;
        }
        ...
        processNotifyVO.setPayment("QFZF");
