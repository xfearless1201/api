 ...
 *  * @Description TODO(卡付支付)
 *  * @Author Vicky
 *  * @Date 2019年06月12日 13:18
 *  * @Version 1.0.0
 *  
 **/
public class KFPayServiceImpl extends PayAbstractBaseService implements PayService {
    private static final Logger logger = LoggerFactory.getLogger(KFPayServiceImpl.class);
    /**
     * 回调失败响应信息
     */
    ...
    public KFPayServiceImpl() {
    }
    public KFPayServiceImpl(Map<String, String> data) {
    ...
    public JSONObject smPay(PayEntity payEntity) {
        try {
            ...
            logger.info("[KF]卡付支付扫码支付请求参数:{}", JSONObject.fromObject(data));
            String resStr = HttpUtils.generatorForm(data, payUrl);
            return PayResponse.sm_form(payEntity, resStr, "下单成功");

        } catch (Exception e) {
            e.printStackTrace();
            return PayResponse.error("[KF]卡付支付支付扫码支付下单异常" + e.getMessage());
        }
    }
    /**
     * @param data
    ...
    private String generatorSign(Map<String, String> data) throws Exception {
        ...
        logger.info("[KF]卡付支付扫码支付生成待签名串:" + signStr);
        String sign = MD5Utils.md5toUpCase_32Bit(signStr);
        logger.info("[KF]卡付支付扫码支付生成MD5加密签名串:" + sign);
        return sign;
    }
    ...
    public boolean serchOrder(String orderNo) {
        try {
            ...
            logger.info("[KF]卡付支付扫码支付回调查询订单{}请求参数：{}", orderNo, JSONObject.fromObject(param));
            String resStr = HttpUtils.toPostForm(param, queryOrderUrl);
            logger.info("[KF]卡付支付扫码支付回调查询订单{}响应信息：{}", orderNo, JSONObject.fromObject(resStr));
            if (StringUtils.isBlank(resStr)) {
                logger.info("[KF]卡付支付扫码支付回调查询订单发起HTTP请求无响应,订单号{}", orderNo);
                return false;
            }
            ...
            if (!"SUCCESS".equalsIgnoreCase(resJson.getString("trade_state"))) {
                logger.info("[KF]卡付支付扫码支付回调查询订单{}订单支付状态：{}", orderNo, resJson.getString("trade_state"));
                return false;
            }
            return true;
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[KF]卡付支付扫码支付回调查询订单{}异常{}", orderNo, e.getMessage());
            return false;
        }
    ...
    private boolean verifyCallback(Map<String, String> data) {
        try {
            ...
            logger.info("[KF]卡付支付扫码支付生成签名串：{}--源签名串：{}", sign, sourceSign);
            return sourceSign.equalsIgnoreCase(sign);
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[KF]卡付支付扫码支付回调生成签名串异常{}", e.getMessage());
            return false;
        }
    }
    ...
    public String notify(HttpServletRequest request, HttpServletResponse response, JSONObject config) {
        ...
        logger.info("[KF]卡付支付扫码支付回调请求参数：" + JSONObject.fromObject(infoMap));
        if (MapUtils.isEmpty(infoMap)) {
            logger.error("KFNotify获取回调请求参数为空");
            return ret__failed;
        }
        //参数验签
        ...
        if (StringUtils.isBlank(order_amount)) {
            logger.info("KFNotify获取实际支付金额为空!");
            return ret__failed;
        }
        ...
        if (!serchOrder(order_no)) {
            logger.info("[KF]卡付支付扫码支付回调查询订单{}失败", order_no);
            return ret__failed;
        }
        /**回调验签*/
