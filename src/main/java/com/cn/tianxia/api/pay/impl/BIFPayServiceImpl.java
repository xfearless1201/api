 ...
 *
 *    Filename:    BIFPayServiceImpl.java 
 *
...
/**
 *  * @ClassName BIFPayServiceImpl
 *  * @Description TODO(币付支付)
 *  * @Author Roman
 *  * @Date 2019年04月29日 17:19
 *  * @Version 1.0.0
 *  
 **/

public class BIFPayServiceImpl extends PayAbstractBaseService implements PayService {
    /**
     * 日志
     */
    private static final Logger logger = LoggerFactory.getLogger(BIFPayServiceImpl.class);
     ...
     * 构造器，初始化参数
     */
    public BIFPayServiceImpl() {
    }

    public BIFPayServiceImpl(Map<String, String> data) {
    ...
    public JSONObject smPay(PayEntity entity) {
        logger.info("[BIF]币付支付扫码支付开始============START======================");
        try {
            //获取支付请求参数
            Map<String, String> data = sealRequest(entity);
            logger.info("[BIF]币付支付扫码支付请求参数:{}", JSONObject.fromObject(data));

            //发送请求
            String response = HttpUtils.generatorFormGet(data, payUrl);
            logger.info("[BIF]币付支付扫码支付响应:{}", response);
            if (StringUtils.isBlank(response)) {
                logger.info("[BIF]币付支付扫码支付发起HTTP请求无响应结果");
                return PayResponse.error("[BIF]币付支付扫码支付发起HTTP请求无响应结果");
            }
            return PayResponse.sm_form(entity, response, "扫码支付下单成功");

        } catch (Exception e) {
            e.printStackTrace();
            return PayResponse.error("[BIF]币付支付扫码支付下单失败" + e.getMessage());
        }
    }
    ...
    private Map<String, String> sealRequest(PayEntity entity) throws Exception {
        logger.info("[BIF]币付支付封装请求参数开始=====================START==================");
        try {
            //创建支付请求参数存储对象
...
//            notifyurl	异步通知地址	是	String(225)	接收平台异步通知回调地址，通知url必须为直接可访问的url，不能携带参数。如： http://www.xxxx.com/wxpay/pay.php
            ...
            e.printStackTrace();
            logger.error("[BIF]币付支付封装请求参数异常:" + e.getMessage());
    ...
    public String generatorSign(Map<String, String> data, int type) throws Exception {
        logger.info("[BIF]币付支付生成支付签名串开始==================START========================");
        try {
...
//                  &attach=yyyxx&key=c4b70b766ea78fe1689f4e4e1afa291a
            ...
            logger.info("[BIF]币付支付生成待签名串:" + singStr);
            //生成加密串
            String sign = MD5Utils.md5toUpCase_32Bit(sb.toString());
            logger.info("[BIF]币付支付生成签名串:" + sign);
            return sign;
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("[BIF]币付支付生成支付签名串异常:" + e.getMessage());
    ...
    private boolean verifyCallback(Map<String, String> data) {
        logger.info("[BIF]币付支付回调验签开始==============START===========");

        //获取回调通知原签名串
        String sourceSign = data.remove("sign");
        logger.info("[BIF]币付支付回调验签获取原签名串:{}", sourceSign);
        //生成验签签名串
        String sign = null;
        try {
            sign = generatorSign(data, 2);
            logger.info("[BIF]币付支付回调验签生成加密串:{}", sign);
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[BIF]币付支付生成加密串异常:{}", e.getMessage());
        }
    ...
    public String notify(HttpServletRequest request, HttpServletResponse response, JSONObject config) {
        ...
        logger.info("[BIF]币付支付回调请求参数:{}", JSONObject.fromObject(infoMap));
        ...
        processNotifyVO.setPayment("BIF");
