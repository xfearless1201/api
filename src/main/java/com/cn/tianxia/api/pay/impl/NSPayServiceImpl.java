 ...
 *
 *    Filename:    NSPayServiceImpl.java 
 *
...
/**
 *  * @ClassName NSPayServiceImpl
 *  * @Description TODO(这里用一句话描述这个类的作用)
 *  * @Author Roman
 *  * @Date 2019年04月07日 11:35
 *  * @Version 1.0.0
 *  
 **/

public class NSPayServiceImpl extends PayAbstractBaseService implements PayService {
    /**
     * 日志
     */
    private static final Logger logger = LoggerFactory.getLogger(NSPayServiceImpl.class);
     ...
     * 构造器，初始化参数
     */
    public NSPayServiceImpl() {
    }

    public NSPayServiceImpl(Map<String, String> data) {
    ...
    public JSONObject smPay(PayEntity entity) {
        try {
            ...
            logger.info("[NS]97支付扫码支付请求参数报文:{}", JSONObject.fromObject(data));

            String response = HttpUtils.generatorForm(data, payUrl);
            logger.info("[NS]97支付扫码支付响应参数：{}", response);
            if (StringUtils.isBlank(response)) {
                logger.info("[NS]97支付扫码支付下单失败，无响应结果");
                return PayResponse.error("[NS]97支付扫码支付下单失败，无响应结果");
            }
            return PayResponse.sm_form(entity, response, "下单成功");
        } catch (Exception e) {
            e.printStackTrace();
            return PayResponse.error("[NS]97支付扫码支付下单异常" + e.getMessage());
        }
    }
    ...
    private Map<String, String> sealRequest(PayEntity entity) throws Exception {
        logger.info("[NS]97支付封装请求参数开始=====================START==================");
        try {
            //创建支付请求参数存储对象
...
//            pay_productname	商品名称	是	否
            ...
            e.printStackTrace();
            logger.error("[NS]97支付封装请求参数异常:" + e.getMessage());
    ...
    public String generatorSign(Map<String, String> data) throws Exception {
        try {
            ...
            logger.info("[NS]97支付生成待签名串:" + signStr);
            //生成加密串
            String sign = MD5Utils.md5toUpCase_32Bit(signStr);
            logger.info("[NS]97支付生成加密签名串:" + sign);
            return sign;
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("[NS]97支付生成支付签名串异常:" + e.getMessage());
    ...
    private boolean verifyCallback(Map<String, String> data) {
        logger.info("[NS]97支付回调验签开始==============START===========");

        //获取回调通知原签名串
        String sourceSign = data.remove("sign");
        logger.info("[NS]97支付回调验签获取原签名串:{}", sourceSign);
        //生成验签签名串
        String sign = null;
        try {
            sign = generatorSign(data);
            logger.info("[NS]97支付回调验签生成加密串:{}", sign);
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[NS]97支付生成加密串异常:{}", e.getMessage());
        }
    ...
    public String notify(HttpServletRequest request, HttpServletResponse response, JSONObject config) {
        ...
        logger.info("[NS]97支付回调请求参数:{}", JSONObject.fromObject(infoMap));
...
//            签名
            ...
            if (respJson.containsKey("returncode") && "00".equals(respJson.getString("returncode"))) {
                if ("SUCCESS".equalsIgnoreCase(respJson.getString("trade_state"))) {
                    ...
                    logger.info("[NS]97支付订单查询成功,订单" + orderNo + "已支付。");
                    return true;
                }
            }
            return false;
