 ...
 *
 *    Filename:    NNNPayServiceImpl.java 
 *
...
/**
 *  * @ClassName NNNPayServiceImpl
 *  * @Description TODO(新牛牛支付)
 *  * @Author Roman
 *  * @Date 2019年04月21日 17:27
 *  * @Version 1.0.0
 *  
 **/

public class NNNPayServiceImpl extends PayAbstractBaseService implements PayService {
    /**
     * 日志
     */
    private static final Logger logger = LoggerFactory.getLogger(NNNPayServiceImpl.class);
     ...
     * 构造器，初始化参数
     */
    public NNNPayServiceImpl() {
    }

    public NNNPayServiceImpl(Map<String, String> data) {
    ...
    public JSONObject smPay(PayEntity entity) {
        try {
            ...
            logger.info("[NNN]新牛牛支付扫码支付请求参数报文:{}", JSONObject.fromObject(data));

            String response = HttpUtils.generatorForm(data, payUrl);
            logger.info("[NNN]新牛牛支付扫码支付响应参数：{}", response);
            if (StringUtils.isBlank(response)) {
                logger.info("[NNN]新牛牛支付扫码支付发起HTTP请求无响应");
                return PayResponse.error("[NNN]新牛牛支付扫码支付发起HTTP请求无响应");
            }
            return PayResponse.sm_form(entity,response,"下单成功");
        } catch (Exception e) {
            e.printStackTrace();
            return PayResponse.error("[NNN]新牛牛支付扫码支付异常" + e.getMessage());
        }
    }
    ...
    private Map<String, String> sealRequest(PayEntity entity) throws Exception {
        logger.info("[NNN]新牛牛支付封装请求参数开始=====================START==================");
        try {
            //创建支付请求参数存储对象
...
//            dataMap.put("format", "json");
            ...
            e.printStackTrace();
            logger.error("[NNN]新牛牛支付封装请求参数异常:" + e.getMessage());
    ...
    public String generatorSign(Map<String, String> data) throws Exception {
        try {
            ...
            logger.info("[NNN]新牛牛支付生成待签名串:" + signStr);
            //生成加密串
            String sign = MD5Utils.md5toUpCase_32Bit(signStr);
            logger.info("[NNN]新牛牛支付生成签名串:" + sign);
            return sign;
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("[NNN]新牛牛支付生成支付签名串异常:" + e.getMessage());
    ...
    private boolean verifyCallback(Map<String, String> data) {
        logger.info("[NNN]新牛牛支付回调验签开始==============START===========");

        //获取回调通知原签名串
        String sourceSign = data.remove("sign");
        logger.info("[NNN]新牛牛支付回调验签获取原签名串:{}", sourceSign);
        //生成验签签名串
        String sign = null;
        try {
            sign = generatorSign(data);
            logger.info("[NNN]新牛牛支付回调验签生成加密串:{}", sign);
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[NNN]新牛牛支付生成加密串异常:{}", e.getMessage());
        }
    ...
    public String notify(HttpServletRequest request, HttpServletResponse response, JSONObject config) {
        ...
        logger.info("[NNN]新牛牛支付回调请求参数:{}", JSONObject.fromObject(infoMap));
        ...
        processNotifyVO.setPayment("NNN");
...
//            签名
            ...
            logger.info("[NNN]新牛牛支付回调订单查询接口请求参数{}", JSONObject.fromObject(map));
            //发送请求
            String response = HttpUtils.toPostForm(map, queryUrl);

            //解析响应参数
            JSONObject respJson = JSONObject.fromObject(response);
            logger.info("[NNN]新牛牛支付回调订单查询接口响应信息{}", respJson);
            if (respJson.containsKey("returncode") && "00".equals(respJson.getString("returncode"))) {
                if ("SUCCESS".equalsIgnoreCase(respJson.getString("trade_state"))) {
                    ...
                    logger.info("[NNN]新牛牛支付回调订单查询成功,订单" + orderNo + "已支付。");
                    return true;
                }
            }
            return false;
