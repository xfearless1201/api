 ...
 *
 *    Filename:    BWINPayServiceImpl.java 
 *
...
/**
 *  * @ClassName BWINPayServiceImpl
 *  * @Description TODO(这里用一句话描述这个类的作用)
 *  * @Author Roman
 *  * @Date 2019年03月24日 21:47
 *  * @Version 1.0.0
 *  
 **/

public class BWINPayServiceImpl extends PayAbstractBaseService implements PayService {

    /**
     * 日志
     */
    private static final Logger logger = LoggerFactory.getLogger(BWINPayServiceImpl.class);
     ...
     * 构造器，初始化参数
     */
    public BWINPayServiceImpl() {
    }

    public BWINPayServiceImpl(Map<String, String> data) {
    ...
    public JSONObject smPay(PayEntity entity) {
        try {
            ...
            logger.info("[BWIN]百胜支付扫码支付响应:{}", response);
            if (StringUtils.isBlank(response)) {
                logger.info("[BWIN]百胜支付扫码支付下单失败，无响应结果");
                return PayResponse.error("[BWIN]百胜支付扫码支付下单失败，无响应结果");
            }
            ...
            return PayResponse.error("[BWIN]百胜支付下单失败:" + response);

        } catch (Exception e) {
            e.printStackTrace();
            return PayResponse.error("[BWIN]百胜支付扫码支付下单异常" + e.getMessage());
        }
    }
    ...
    private Map<String, String> sealRequest(PayEntity entity) throws Exception {
        logger.info("[BWIN]百胜支付封装请求参数开始=====================START==================");
        try {
            //创建支付请求参数存储对象
...
//            ChannelType	Int	是	接入类型，10：二维码转帐
...
//            Sign	String/64	是	签名(MD5加密)
            ...
            e.printStackTrace();
            logger.error("[BWIN]百胜支付封装请求参数异常:" + e.getMessage());
    ...
    public String generatorSign(Map<String, String> data, int type) throws Exception {
        logger.info("[BWIN]百胜支付生成支付签名串开始==================START========================");
        try {
...
//                MD5(MerchantNo + OutTradeNo + (商户秘钥)).ToLower ()
                ...
                sb.append(data.get("OutTradeNo"));
            }
            sb.append(key);
            //生成待签名串
            String signStr = sb.toString();
            logger.info("[BWIN]百胜支付生成待签名串:" + signStr);
            //生成加密串
            String sign = Objects.requireNonNull(MD5Utils.md5toUpCase_32Bit(signStr)).toLowerCase();
            logger.info("[BWIN]百胜支付生成加密签名串:" + sign);
            return sign;
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("[BWIN]百胜支付生成支付签名串异常:" + e.getMessage());
    ...
    private boolean verifyCallback(Map<String, String> data) {
        logger.info("[BWIN]百胜支付回调验签开始==============START===========");

        //获取回调通知原签名串
        String sourceSign = data.remove("Sign");
        logger.info("[BWIN]百胜支付回调验签获取原签名串:{}", sourceSign);
        //生成验签签名串
        String sign = null;
        try {
            sign = generatorSign(data, 2);
            logger.info("[BWIN]百胜支付回调验签生成加密串:{}", sign);
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[BWIN]百胜支付生成加密串异常:{}", e.getMessage());
        }
...
//            签名
            ...
            logger.info("[BWIN]百胜支付订单查询接口订单{}请求参数{}", orderNo, map);
            //发送请求
            String response = HttpUtils.toPostForm(map, queryUrl);
            logger.info("[BWIN]百胜支付订单查询接口响应信息{}", response);
            ...
            if (respJson.containsKey("Code") && "1000".equals(respJson.getString("Code"))) {
                if ("1".equals(respJson.getJSONObject("Data").getString("Status"))) {
                    ...
                    logger.info("[BWIN]百胜支付订单查询成功,订单" + orderNo + "已支付。");
                    return true;
                }
            }
            return false;
    ...
    public String notify(HttpServletRequest request, HttpServletResponse response, JSONObject config) {
        ...
        logger.info("[BWIN]百胜支付回调请求参数:{}", infoMap);
        ...
        processNotifyVO.setPayment("BWIN");
