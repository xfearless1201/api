 ...
 *
 *    Filename:    FYJHPayServiceImpl.java 
 *
...
/**
 *  * @ClassName FYJHPayServiceImpl
 *  * @Description TODO(这里用一句话描述这个类的作用)
 *  * @Author Roman
 *  * @Date 2019年04月05日 15:02
 *  * @Version 1.0.0
 *  
 **/

public class FYJHPayServiceImpl extends PayAbstractBaseService implements PayService {
    /**
     * 日志
     */
    private static final Logger logger = LoggerFactory.getLogger(FYJHPayServiceImpl.class);
     ...
     * 构造器，初始化参数
     */
    public FYJHPayServiceImpl() {
    }

    public FYJHPayServiceImpl(Map<String, String> data) {
    ...
    public JSONObject wyPay(PayEntity entity) {
        logger.info("[FYJH]风云聚合支付网银支付开始============START======================");
        try {
            //获取请求参数
            Map<String, String> data = sealRequest(entity);
            logger.info("[FYJH]风云聚合支付网银支付请求参数{}", data);
            //发送请求
            String response = HttpUtils.generatorForm(data, payUrl);

            logger.info("[FYJH]风云聚合支付网银支付响应结果{}", response);
            if (StringUtils.isBlank(response)) {
                logger.info("[FYJH]风云聚合支付网银支付发起请求无响应结果");
                return PayResponse.error("[FYJH]风云聚合支付网银支付发起请求无响应结果");
            }
            return PayResponse.wy_form(entity.getPayUrl(), response);
        } catch (Exception e) {
            e.printStackTrace();
            return PayResponse.error("[FYJH]风云聚合支付网银支付下单失败" + e.getMessage());
        }
    }
    ...
    public JSONObject smPay(PayEntity entity) {
        try {
            //获取支付请求参数
            Map<String, String> data = sealRequest(entity);
            logger.info("[FYJH]风云聚合支付扫码支付请求参数:{}", data);

            String resStr = HttpUtils.generatorForm(data, payUrl);
            logger.info("[FYJH]风云聚合支付扫码支付响应信息：{}", resStr);
            if (StringUtils.isBlank(resStr)) {
                logger.info("[FYJH]风云聚合支付扫码支付下单失败，无响应结果");
                return PayResponse.error("[FYJH]风云聚合支付扫码支付下单失败，无响应结果");
            }
            return PayResponse.sm_form(entity, resStr, "扫码支付下单成功");

        } catch (Exception e) {
            e.printStackTrace();
            return PayResponse.error("[FYJH]风云聚合支付扫码支付下单异常" + e.getMessage());
        }
    }
    ...
    private Map<String, String> sealRequest(PayEntity entity) throws Exception {
        logger.info("[FYJH]风云聚合支付封装请求参数开始=====================START==================");
        try {
            //创建支付请求参数存储对象
            ...
            e.printStackTrace();
            logger.error("[FYJH]风云聚合支付封装请求参数异常:" + e.getMessage());
...
//              签名：  order_id=xxx&amount=xxx&verified_time=xxx
            ...
            logger.info("[FYJH]风云聚合支付生成待签名串:" + signStr);
            //生成加密串
            ...
            logger.info("[FYJH]风云聚合支付生成加密签名串:" + sign);
            return sign;
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("[FYJH]风云聚合支付生成支付签名串异常:" + e.getMessage());
    ...
    private boolean verifyCallback(Map<String, String> data) {
        logger.info("[FYJH]风云聚合支付回调验签开始==============START===========");

        //获取回调通知原签名串
        String sourceSign = data.remove("qsign");
        logger.info("[FYJH]风云聚合支付回调验签获取原签名串:{}", sourceSign);
        //生成验签签名串
        String sign = null;
        try {
            sign = generatorSign(data, 2);
            logger.info("[FYJH]风云聚合支付回调验签生成加密串:{}", sign);
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[FYJH]风云聚合支付生成加密串异常:{}", e.getMessage());
        }
    ...
    private boolean getOrderStatus(String orderNo) {
        try {
            ...
            logger.info("[FYJH]风云聚合支付订单查询接口请求参数{}", JSONObject.fromObject(map));
            ...
            logger.info("[FYJH]风云聚合支付订单查询接口响应信息{}", respJson);
            if (respJson.containsKey("code") && "200".equals(respJson.getString("code"))) {
                if ("verified".equalsIgnoreCase(respJson.getJSONObject("order").getString("status"))) {
                    ...
                    logger.info("[FYJH]风云聚合支付订单查询成功,订单" + orderNo + "已支付。");
                    return true;
                }
            }
            return false;
    ...
    public String notify(HttpServletRequest request, HttpServletResponse response, JSONObject config) {
        ...
        logger.info("[FYJH]风云聚合支付回调请求参数:{}", JSONObject.fromObject(infoMap));
        ...
        // 支付商订单号
        String tradeNo = "fyjh" + System.currentTimeMillis();
        //订单状态
        ...
        processNotifyVO.setPayment("FYJH");
