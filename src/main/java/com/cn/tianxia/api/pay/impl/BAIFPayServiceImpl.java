...
/**
 *  * @ClassName BAIFPayServiceImpl
 *  * @Description TODO(百分支付)
 *  * @Author Bing
...
public class BAIFPayServiceImpl extends PayAbstractBaseService implements PayService {
    /**
     * 日志
     */
    private static final Logger logger = LoggerFactory.getLogger(BAIFPayServiceImpl.class);
     ...
     * 构造器，初始化参数
     */
    public BAIFPayServiceImpl() {
    }

    public BAIFPayServiceImpl(Map<String, String> data) {
    ...
    public JSONObject smPay(PayEntity entity) {
        try {
            ...
            logger.info("[BAIF]百分扫码支付请求参数报文:{}", JSONObject.fromObject(data));

            String response = HttpUtils.generatorFormGet(data, payUrl);
            logger.info("[BAIF]百分扫码支付响应参数：{}", response);
            if (StringUtils.isBlank(response)) {
                logger.info("[BAIF]百分扫码支付发起HTTP请求无响应");
                return PayResponse.error("[BAIF]百分扫码支付发起HTTP请求无响应");
            }
            return PayResponse.sm_form(entity, response, "下单成功");
        } catch (Exception e) {
            e.printStackTrace();
            return PayResponse.error("[BAIF]百分扫码支付异常" + e.getMessage());
        }
    }
    ...
    private Map<String, String> sealRequest(PayEntity entity) throws Exception {
        logger.info("[BAIF]百分扫码支付封装请求参数开始=====================START==================");
        try {
            //创建支付请求参数存储对象
...
//            sign	签名	详见1.1安全规范
            ...
            e.printStackTrace();
            logger.error("[BAIF]百分扫码支付封装请求参数异常:" + e.getMessage());
            throw new Exception("[BAIF]百分扫码支付封装支付请求参数异常!");
        }
    }
    ...
    public String generatorSign(Map<String, String> data) throws Exception {
        try {
            ...
            logger.info("[BAIF]百分扫码支付生成待签名串:" + signStr);
            //生成加密串
            String sign = MD5Utils.md5toUpCase_32Bit(signStr).toLowerCase();
            logger.info("[BAIF]百分扫码支付生成签名串:" + sign);
            return sign;
        } catch (Exception e) {
            e.printStackTrace();
            logger.error("[BAIF]百分扫码支付生成支付签名串异常:" + e.getMessage());
            throw new Exception("[BAIF]百分扫码支付生成支付签名串异常!");
        }
    }
    ...
    private boolean verifyCallback(Map<String, String> data) {
        logger.info("[BAIF]百分回调验签开始==============START===========");

        //获取回调通知原签名串
        String sourceSign = data.remove("sign");
        logger.info("[BAIF]百分扫码支付回调验签获取原签名串:{}", sourceSign);
        //生成验签签名串
        String sign = null;
        try {
            sign = generatorSign(data);
            logger.info("[BAIF]百分扫码支付回调验签生成加密串:{}", sign);
        } catch (Exception e) {
            e.printStackTrace();
            logger.info("[BAIF]百分扫码支付回调生成加密串异常:{}", e.getMessage());
        }
    ...
    public String notify(HttpServletRequest request, HttpServletResponse response, JSONObject config) {
        ...
        logger.info("[BAIF]百分扫码支付回调请求参数:{}", JSONObject.fromObject(infoMap));
        ...
        // 第三方订单号
        String tradeNo = "BAIF" + System.currentTimeMillis();
        //订单状态
        ...
        processNotifyVO.setPayment("BAIF");
...
//            签名
            ...
            e.printStackTrace();
            logger.info("[BAIF]百分扫码支付回调订单查询异常");
            return false;
        }
    }
}
